...
def new_engine(emb: Embeddings, target_idx=None, max_queries=HUMAN_MAX_QUERIES):
    rng = np.random.default_rng()
    return GammaCKLEngine(
        X=emb.X,
        gamma=config.GAMMA,
...
def render_history_with_images(emb: Embeddings):
    ...
    for rec in hist:
        ...
        choice = rec.get("choice")
        if choice == "A":
            a_hl, b_hl, verdict = "chosen", "rejected", "picked A"
        elif choice == "B":
            a_hl, b_hl, verdict = "rejected", "chosen", "picked B"
        else:
...
def screen_home(emb: Embeddings):
    ...
    st.caption(f"Search algorithm: γ-CKLSearch (Chumbalov et al. 2024), γ={config.GAMMA}")
    st.markdown("")

    col1, col2 = st.columns(2)
    with col1:
        if st.button("Start search (human)", type="primary", use_container_width=True):
            st.session_state.engine = new_engine(emb, target_idx=None, max_queries=HUMAN_MAX_QUERIES)
    ...
    with col2:
        if st.button("Auto-run (simulated user)", type="primary", use_container_width=True):
            ...
            st.session_state.engine = new_engine(emb, target_idx=target, max_queries=AUTO_MAX_QUERIES)
            st.session_state.history = []
...
def screen_human_search(emb: Embeddings):
    engine: GammaCKLEngine = st.session_state.engine
